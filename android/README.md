...
- **WiFi 配网**：扫描附近 2.4GHz WiFi，下发 SSID、密码、静态 IP、网关、子网和 DNS
...
2. `detectForVideo` 时间戳改为单调递增，避免时间戳异常导致检测不稳定。

若仍识别差，优先检查：

- 光照是否充足、背景是否干净；
- 手是否完整进入取景框；
- 首次启动后是否完成三步校准（张开/握拳/拇指内收）；
...
3. 从扫描列表选择目标 2.4GHz SSID，输入密码
4. 填写静态 IP、网关、子网、DNS1、DNS2；默认值已补全，通常只需要修改静态 IP
5. 点击“下发并切 STA”后，ESP32 会保存配置并切换到目标 WiFi
6. 手机切到同一个 WiFi，在 Host 填入静态 IP 后重新连接控制
